...
export interface TransactionFilters {
  category?: string;
  period?: string;
  party?: string; // Renamed from merchant to party (can be from or to)
  type?: string;
}
...
export interface FilterOptions {
  categories: string[];
  parties: string[]; // Renamed from merchants to parties
  types: string[];
  periods: string[];
}
...
async function getFilteredTransactions(
  ...
  console.log(`Filtered ${data?.length || 0} transactions to ${limitedData.length} results`);
  return limitedData;
}
